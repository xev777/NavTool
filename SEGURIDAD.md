...
| S3 | **Elevación desde una carpeta modificable.** En modo administrador (Npcap), si NavTool vive en una carpeta escribible por tu usuario —o se extrae a `%TEMP%`, como hace un `.exe` único de PyInstaller— cualquier programa tuyo podría alterarlo y ejecutar con permisos elevados. | Revisión de diseño. | Compilación en modo **carpeta** (`--onedir`, sin extraer a temporales), script `Instalar-NavTool.ps1` que instala en *Archivos de programa* y verifica permisos, y un aviso antes de elevar si estás fuera de esa ubicación. |
...
| S14 | El historial (sitios visitados) se guardaba en la carpeta *Roaming*, que se sincroniza en perfiles móviles. | Ahora en `%LOCALAPPDATA%\NavTool`, con migración, botón **Borrar todo el historial** y retención de 30 días. |
...
2. **Un programa malicioso con tus mismos permisos** puede leer `history.db` y la configuración, cambiar el proxy de Windows o cerrar NavTool. NavTool no es una defensa contra malware que ya corre como tú. El historial contiene los sitios que visitas: bórralo si no lo quieres.
...
4. **Los binarios no están firmados.** Windows SmartScreen puede avisar. `Instalar-NavTool.ps1` muestra el SHA-256 al instalar para que puedas comprobarlo después.
...
| S22 | **Instalador sin marca UTF-8:** Inno Setup lo leía como ANSI y sus mensajes con tildes salían rotos («¿» → «Â¿»). | `NavTool.iss`, `compilar.ps1` e `Instalar-NavTool.ps1` guardados con BOM UTF-8. |
| S23 | **La captura perdía la mayor parte del tráfico** en descargas rápidas (scapy: ~4.000 paquetes/s). Con una ISO de 6 GB solo se veía el 6 %. | Lector propio de Npcap (búfer de 16 MB, copia directa): exacto contra los contadores de la tarjeta (966 MB vs 966 MB). Aviso si NavTool ve menos del 85 % de la tarjeta. |
| S24 | Con el proxy activo, todo el tráfico de los navegadores aparecía como `NavTool.exe`. | El proxy anota qué navegador pidió cada conexión y el monitor muestra su nombre. |
...
| | Instalador (`NavTool-Setup-1.0.exe`) | Portable (`NavTool-Portable-1.0.zip`) |
|---|---|---|
| Dónde vive | *Archivos de programa* (solo un administrador puede modificarlo) | Donde la extraigas |
| Datos (config., historial) | `%LOCALAPPDATA%\NavTool` | Carpeta `Datos` junto al programa |
| Registro de Windows | Solo el inicio automático, si lo eliges (HKCU) | Nada |
| Modo administrador (Npcap) | Seguro (ver S3) | Solo seguro si la carpeta no la puede modificar otro programa tuyo; NavTool avisa |
...
```powershell
powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1                    # instala en Archivos de programa
powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1 -InicioConWindows  # además, arranca con tu sesión
powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1 -Desinstalar       # quitar (conserva tu historial)
```
