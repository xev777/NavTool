<#
.SYNOPSIS
    Instala NavTool en «Archivos de programa», una carpeta que solo un administrador puede modificar.

.DESCRIPTION
    Por qué importa: NavTool pide permisos de administrador para capturar el tráfico (Npcap).
    Si el programa vive en una carpeta que cualquier proceso de tu usuario puede modificar,
    ese proceso podría cambiar los archivos de NavTool y ejecutarse con permisos elevados
    ...
    Qué hace:
      ...
      2. Copia la carpeta dist\NavTool a C:\Program Files\NavTool.
      3. Comprueba los permisos: si el grupo «Users» pudiera escribir ahí, lo corrige.
      4. Crea un acceso directo en el menú Inicio.
      5. (Opcional) -InicioConWindows: NavTool arranca al iniciar sesión, oculto en la bandeja.
    No toca tu configuración ni tu historial (están en %LOCALAPPDATA%\NavTool).

.PARAMETER Origen
    Carpeta con NavTool.exe (por defecto: dist\NavTool junto a este script).
.PARAMETER InicioConWindows
    Añade NavTool al inicio de tu sesión.
.PARAMETER Desinstalar
    Quita NavTool, el acceso directo y el inicio automático.
.PARAMETER BorrarDatos
    Con -Desinstalar: también borra tu configuración e historial (%LOCALAPPDATA%\NavTool).

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1 -InicioConWindows
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Instalar-NavTool.ps1 -Desinstalar
#>
[CmdletBinding()]
param(
    [string]$Origen = (Join-Path $PSScriptRoot "dist\NavTool"),
...
$destino = Join-Path $env:ProgramFiles "NavTool"
$acceso  = Join-Path ([Environment]::GetFolderPath("CommonPrograms")) "NavTool.lnk"
$runKey  = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
$datos   = Join-Path $env:LOCALAPPDATA "NavTool"
...
function Detener-NavTool {
    Get-Process -Name NavTool -ErrorAction SilentlyContinue | Stop-Process -Force
...
if ($Desinstalar) {
    ...
    Remove-ItemProperty -Path $runKey -Name "NavTool" -ErrorAction SilentlyContinue
    if ($BorrarDatos -and (Test-Path -LiteralPath $datos)) { Remove-Item -LiteralPath $datos -Recurse -Force }
    Write-Host "NavTool desinstalado." -ForegroundColor Green
    if (-not $BorrarDatos) { Write-Host "Tu configuración e historial se conservaron en: $datos" }
    return
}

# ---- 2) copiar
$exeOrigen = Join-Path $Origen "NavTool.exe"
if (-not (Test-Path -LiteralPath $exeOrigen)) {
    throw "No encuentro NavTool.exe en '$Origen'. Compila primero (carpeta dist\NavTool) o usa -Origen."
}
Detener-NavTool
if (Test-Path -LiteralPath $destino) { Remove-Item -LiteralPath $destino -Recurse -Force }
...
$exe = Join-Path $destino "NavTool.exe"
$sh = New-Object -ComObject WScript.Shell
$lnk = $sh.CreateShortcut($acceso)
$lnk.TargetPath = $exe
$lnk.WorkingDirectory = $destino
$lnk.Description = "NavTool: barra de utilidades de navegación y red"
$lnk.Save()
Write-Host "Acceso directo creado en el menú Inicio."

# ---- 5) inicio con Windows (opcional, solo para tu usuario)
if ($InicioConWindows) {
    Set-ItemProperty -Path $runKey -Name "NavTool" -Value "`"$exe`" --segundo-plano"
    Write-Host "NavTool arrancará al iniciar tu sesión (en la bandeja del sistema)."
}

$hash = (Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash
Write-Host ""
Write-Host "Instalación terminada." -ForegroundColor Green
Write-Host "SHA-256 de NavTool.exe: $hash"
Write-Host "(Guárdalo: si algún día cambia sin que hayas actualizado, algo modificó el programa.)"
Write-Host "Ábrelo desde el menú Inicio → NavTool."
